...
[AnEntrypoint/agentplug-libsql](https://github.com/AnEntrypoint/agentplug-libsql), whose `.github/workflows/release.yml` calls the shared reusable workflow `wasm-plugin-release.yml` hosted in [AnEntrypoint/rs-plugkit](https://github.com/AnEntrypoint/rs-plugkit). A push to `main` touching `src/**` or `Cargo.toml` installs the WASI SDK, builds for `wasm32-wasip1`, and publishes a release tagged `v<version>` titled `agentplug-libsql v<version>`, where the version is read from the crate's own `Cargo.toml`.
